...
// **Função para desenhar o cubo 3D**
...
// **Função para desenhar a grade inferior (malha 3D)**
void draw3DGrid(RotationMatrix rotationMatrix, float scale)
{
  int screenWidth = M5.Display.width();
  int screenHeight = M5.Display.height();
  int centerX = screenWidth / 2;
  int centerY = screenHeight - 30; // Posiciona a malha mais abaixo
  ...
  for (int i = -10; i <= 10; i++)
  {
    for (int j = -10; j <= 10; j++)
    {
      ...
      // Gradiente de cinza claro para cinza escuro
